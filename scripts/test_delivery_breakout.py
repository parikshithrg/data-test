"""Phase 3, signal 1: delivery-confirmed breakout, tested under this
project's rules.

    python scripts/test_delivery_breakout.py --split delivery --window train

Same harness as `test_mean_reversion.py` (bhav store -> point-in-time
...
-> hypothesis log), with one addition: a delivery_pct panel pulled from
`fno.db`'s `cash_delivery_daily` (`dtest.data.delivery`), reindexed onto the
price panel's own calendar/symbol set.

Only `--split delivery` is meaningful here - `delivery_pct` is NaN before
2019-06-27 by construction (see `dtest.data.delivery.EARLIEST_DATE`), so a
`primary`/train run would legitimately produce zero signals, not a useful
test. The CLI still accepts `--split primary` for that reason: an
all-primary run that reports 0 firings is a good sanity check, not just a
foot-gun.
"""
...
STORY = (
    "A price breakout on ordinary or low delivery is disproportionately "
    "intraday speculation squaring off the same day - momentum with no one "
    "actually taking the shares home, prone to reverse once that flow "
    "unwinds overnight. A breakout where delivery_pct sits meaningfully "
    "above its own recent normal is disproportionately real buyers "
    "converting the move into settled positions, which the market then has "
    "to keep absorbing rather than give back. market_gate blended delivery "
    "into a multi-term composite (its participation term) but never tested "
    "it alone, under honest execution, as a standalone signal."
)
...
def main() -> int:
    ...
    print(f"=== delivery_breakout ({args.split}/{args.window}) ===")
    ...
    print("loading delivery data from fno.db ...")
    deliv_long = load_delivery_long(cfg.paths.fno_db)
    deliv_panel = to_panel(deliv_long, "delivery_pct")
    print(f"delivery panel: {deliv_panel.shape[0]} sessions x {deliv_panel.shape[1]} symbols "
         f"({deliv_panel.index[0].date()} .. {deliv_panel.index[-1].date()})")
    ...
    # Delivery data has its own symbol/date coverage - reindexed onto the
    # price panel's calendar/columns, same "callers own alignment" division
    # of responsibility the signal module's docstring states. A symbol or
    # date missing from delivery becomes NaN here, which rolling_zscore
    # already treats as "cannot confirm" (see that function's std>0 guard),
    # not as a free pass.
    deliv_aligned = deliv_panel.reindex(index=stock_close.index, columns=stock_close.columns)
    coverage = deliv_aligned.notna().to_numpy().mean()
    print(f"  delivery coverage after reindex: {coverage:.1%} of (date,symbol) cells")
    ...
    out_dir = Path(cfg.paths.runs) / f"delivery_breakout_{args.split}_{args.window}"
    ...
    entry = HypothesisEntry(
        title="delivery_breakout (honest execution)", story=STORY,
        ...
        notes=f"portfolio Sharpe {pm['sharpe']:.3f}, CAGR {pm['cagr_pct']:.2f}%, "
             f"max DD {pm['max_drawdown_pct']:.2f}%, delivery coverage {coverage:.1%}",
    )
