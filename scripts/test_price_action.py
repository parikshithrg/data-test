"""Phase 4/5: price-action long side, tested honestly from the start - no
new infrastructure needed, unlike the short side (see
`scripts/diagnostic_price_action_short.py`), since the long leg trades
cash equity through the exact same `engine/simulate.py` pipeline every
other signal in this project already uses.

    python scripts/test_price_action.py --split primary --window train

Tests ONLY the long half of `signals/price_action.py`'s wide-range
conviction bar (close pinned near the bar's high, on unusual range and
volume). The short half needs a futures short simulator that does not
exist yet - screened separately, cheaply, before that gets built.
"""
...
STORY = (
    "An ordinary session has a normal range, and most wide-range sessions "
    "are still indecisive - price whips both directions and settles back "
    "near the middle, telling you nothing except that the day was "
    "volatile. A session that is BOTH unusually wide AND closes pinned at "
    "one extreme is a different claim: participants pushed the price one "
    "way and never let it come back, all session, on volume well above "
    "what that stock normally trades - disproportionately genuine new "
    "information or real institutional participation, not noise. "
    "Deliberately a different shape from vol_squeeze_breakout (which "
    "needs a prior contraction) - this signal requires nothing about what "
    "came before, only that today's own bar is the outlier."
)
...
def main() -> int:
    ...
    print(f"=== price_action LONG ({args.split}/{args.window}) ===")
    ...
    out_dir = Path(cfg.paths.runs) / f"price_action_long_{args.split}_{args.window}"
    ...
    entry = HypothesisEntry(
        title="price_action LONG (honest execution)", story=STORY,
        ...
        notes=f"portfolio Sharpe {pm['sharpe']:.3f}, CAGR {pm['cagr_pct']:.2f}%, "
             f"max DD {pm['max_drawdown_pct']:.2f}% - LONG side only, short side "
             f"screened separately (no honest short simulator yet)",
    )
