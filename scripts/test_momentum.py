"""Phase 5, signal 1: 12-1 month cross-sectional momentum, long-only, tested
under this project's rules.

    python scripts/test_momentum.py --split primary --window train

Same harness as `test_vol_squeeze_breakout.py` (bhav store -> point-in-time
universe -> signal -> trade simulator -> sizing-independent metrics ->
benchmark excess -> placebo noise floor -> portfolio account simulation ->
hypothesis log) - momentum needs no bespoke lifecycle: `ExitRule` with
`atr_stop_multiple=None` is already a pure calendar hold, which is exactly
what "enter at each monthly rebalance, hold ~1 month" is. See
`dtest/signals/momentum.py` for the full story and the 12-1 construction.
"""
...
HOLD_DAYS = 21   # ~1 month - re-rank and re-enter at the next rebalance

STORY = (
    "A stock that has outperformed for the trailing ~12 months (skipping "
    "the most recent month) is more likely still mid-diffusion of genuine "
    "improving information than already fully priced - institutions build "
    "and unwind large positions over months, not days, so the market's "
    "own adjustment to real news is gradual, not instant. The trader on "
    "the other side, fading a year-long trend on no more information than "
    "'it went up a lot', is betting the market has been wrong for a year "
    "and is about to correct - a strong claim with no specific catalyst "
    "behind it. Unlike every Phase 1-4 signal in this project (all react "
    "to a DAYS-scale dislocation and bet on what happens next over the "
    "same short horizon - and all failed the same way, entering into the "
    "tail of an unfinished move), this is long-horizon by construction: "
    "the 1-month skip specifically excludes that same short-term reversal "
    "window this project's own entry-timing diagnostic found to be real."
)
...
def main() -> int:
    ...
    print(f"=== momentum (12-1 month, long-only) ({args.split}/{args.window}) ===")
    ...
    out_dir = Path(cfg.paths.runs) / f"momentum_{args.split}_{args.window}"
    ...
    entry = HypothesisEntry(
        title="momentum (12-1 month, long-only, honest execution)", story=STORY,
