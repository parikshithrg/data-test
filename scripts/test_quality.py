"""Phase 6, signal 3: quality / profitability trend, long-only, tested
under this project's rules - third fundamentals-based hypothesis.

    python scripts/test_quality.py --split primary --window train

Same harness as `test_earnings_surprise.py` (event-fired signal, calendar
hold, no price data in the trigger). `HOLD_DAYS=60`, matching earnings_
surprise's own PEAD-length window - both are "does the market underreact
to a real, dated fundamental disclosure" claims, so a shared hold length
keeps them comparable rather than confounding the comparison with a
different holding period. See `dtest/signals/quality.py` for the full
story and why this is structurally different from both other fundamentals
signals (no price data in the trigger at all, unlike value.py's P/E).
"""
...
from dtest.features.fundamentals import margin_trend_zscore
from dtest.signals.quality import TREND_THRESHOLD, quality_signal
...
STORY = (
    "A company whose trailing net profit margin is expanding faster than "
    "its own recent normal is disproportionately doing something "
    "structurally better - pricing power, cost discipline, an improving "
    "mix - and that operational improvement takes the market time to "
    "fully re-rate into the price, the same information-diffuses-"
    "gradually logic momentum.py uses for a price trend and earnings_"
    "surprise.py uses for a one-quarter EPS surprise, here applied to a "
    "MULTI-QUARTER margin trend instead. Structurally different from both "
    "other fundamentals signals tested in this project: it uses NO price "
    "data in its own trigger at all (unlike value.py's P/E, which is "
    "flagged as mostly price-driven day to day), and it reacts to a "
    "multi-quarter TREND (TTM margin now vs TTM margin a year ago) rather "
    "than earnings_surprise's single-quarter surprise."
)
...
def _build_trend_per_symbol(cfg, symbols: list[str]) -> dict[str, pd.Series]:
    ...
    for symbol in symbols:
        ...
        if df.empty:
            continue
        trend = margin_trend_zscore(df)
        if trend.notna().any():
            out[symbol] = trend
            n_with_data += 1
    print(f"  margin-trend series built for {n_with_data}/{len(symbols)} symbols")
    return out
...
def main() -> int:
    ...
    ap.add_argument("--threshold", type=float, default=TREND_THRESHOLD)
    args = ap.parse_args()
    ...
    print(f"=== quality / profitability trend ({args.split}/{args.window}) ===")
    ...
    print("building point-in-time margin-trend series per symbol ...")
    trend_per_symbol = _build_trend_per_symbol(cfg, stocks)

    raw_signal = quality_signal(trend_per_symbol, close.index, threshold=args.threshold)
    ...
    out_dir = Path(cfg.paths.runs) / f"quality_{args.split}_{args.window}"
    ...
    entry = HypothesisEntry(
        title="quality (margin trend, long-only, honest execution)", story=STORY,
        ...
        notes=f"portfolio Sharpe {pm['sharpe']:.3f}, CAGR {pm['cagr_pct']:.2f}%, "
             f"max DD {pm['max_drawdown_pct']:.2f}%, trend_threshold={args.threshold}, "
