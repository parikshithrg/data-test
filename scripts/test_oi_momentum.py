"""Phase 3, signal 2: OI-confirmed momentum, tested under this project's
rules.

    python scripts/test_oi_momentum.py --split primary --window train

Same harness as `test_mean_reversion.py`/`test_delivery_breakout.py`, with
a front-month stock-futures OI panel pulled from `fno.db`'s
`fno_bhavcopy_full` (`dtest.data.fno_oi`), reindexed onto the price panel's
own calendar/symbol set.

Runs on `primary` (OI available from 2008, per config.toml's own comment on
that split) - `delivery` would also work mechanically but the signal has
nothing to do with delivery data, so there is no reason to accept that
split's much shorter history for it.
"""
...
STORY = (
    "A price breakout on falling or merely average open interest is "
    "disproportionately short-covering or spot-only buying with no fresh "
    "leveraged conviction behind it. A breakout accompanied by open "
    "interest rising meaningfully faster than its own recent normal means "
    "participants are opening NEW leveraged positions into the move, more "
    "costly to reverse than an unlevered spot rally, disproportionately "
    "real conviction rather than noise. Filtered to days_to_expiry in "
    "[5, 25] to remove the measured post-rollover ramp and pre-expiry "
    "rolldown mechanical effects before testing whether anything real "
    "remains."
)
...
def main() -> int:
    ...
    print(f"=== oi_momentum ({args.split}/{args.window}) ===")
    ...
    print("loading front-month stock-futures OI from fno.db ...")
    oi_long = load_front_month_oi(cfg.paths.fno_db)
    oi_chg_panel = to_panel(oi_long, "oi_chg_pct")
    dte_panel = to_panel(oi_long, "days_to_expiry")
    print(f"OI panel: {oi_chg_panel.shape[0]} sessions x {oi_chg_panel.shape[1]} symbols "
         f"({oi_chg_panel.index[0].date()} .. {oi_chg_panel.index[-1].date()})")
    ...
    oi_chg_aligned = oi_chg_panel.reindex(index=stock_close.index, columns=stock_close.columns)
    dte_aligned = dte_panel.reindex(index=stock_close.index, columns=stock_close.columns)
    coverage = oi_chg_aligned.notna().to_numpy().mean()
    print(f"  OI coverage after reindex: {coverage:.1%} of (date,symbol) cells "
         f"(zero by construction before 2008 and for names with no listed futures)")

    print("building point-in-time universe ...")
    uni = build_universe(stock_close, stock_turnover, cfg)

    raw_signal = oi_momentum_signal(stock_close, oi_chg_aligned, dte_aligned)
    ...
    out_dir = Path(cfg.paths.runs) / f"oi_momentum_{args.split}_{args.window}"
    ...
    entry = HypothesisEntry(
        title="oi_momentum (honest execution)", story=STORY,
        ...
        notes=f"portfolio Sharpe {pm['sharpe']:.3f}, CAGR {pm['cagr_pct']:.2f}%, "
             f"max DD {pm['max_drawdown_pct']:.2f}%, OI coverage {coverage:.1%}",
    )
