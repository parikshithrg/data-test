"""DIAGNOSTIC ONLY - quantitative market-regime check on the 5 stocks that
repeated across multiple strategies' best-5 lists (INDIAINFO, RUSHIL,
IBREALEST, COREPROTEC, JINDALSTEL - see `analyze_stock_performance.py`).
Asks: was the outperformance stock-specific, or did it ride (a) a genuine
sector-wide rally (local), (b) NIFTY50 strength (local), (c) a calm global
macro backdrop (global, reusing the stress composite), or (d) a broad
secular run any reasonably-timed entry would have caught regardless of the
signal?

    python scripts/diagnose_top_performers.py

No new backtest - reuses the real, already-saved trades for each
(symbol, strategy) cell, plus the full price panel and the stress composite
already built for the cross-asset stress-gate work, to characterize the
market backdrop AT EACH WINNING TRADE and compare the stock's own return
over that exact window against (a) its same-sector peers over the identical
window and (b) NIFTY50 over the identical window - isolating stock-specific
alpha from riding a sector or market move. NOT logged to hypothesis_log.csv
- descriptive, not a new hypothesis.
"""
...
# (symbol, strategy, trades file relative to runs/, kind, date_col, exit_col, ret_col)
CELLS = [
    ("RUSHIL", "mean_reversion", "mean_reversion_primary_train/trades.csv",
     "single", "entry_date", "exit_date", "net_pnl_pct"),
    ("INDIAINFO", "oi_momentum", "oi_momentum_primary_train/trades.csv",
     "single", "entry_date", "exit_date", "net_pnl_pct"),
    ("IBREALEST", "oi_momentum", "oi_momentum_primary_train/trades.csv",
     "single", "entry_date", "exit_date", "net_pnl_pct"),
    ("IBREALEST", "participant_tilt_stress_gated",
     "participant_tilt_stress_gated_delivery_train/trades.csv",
     "single", "entry_date", "exit_date", "net_pnl_pct"),
    ("COREPROTEC", "vol_squeeze_breakout", "vol_squeeze_breakout_primary_train/trades.csv",
     "single", "entry_date", "exit_date", "net_pnl_pct"),
    ("INDIAINFO", "vol_squeeze_breakout", "vol_squeeze_breakout_primary_train/trades.csv",
     "single", "entry_date", "exit_date", "net_pnl_pct"),
    ("COREPROTEC", "price_action_long", "price_action_long_primary_train/trades.csv",
     "single", "entry_date", "exit_date", "net_pnl_pct"),
    ("RUSHIL", "price_action_long", "price_action_long_primary_train/trades.csv",
     "single", "entry_date", "exit_date", "net_pnl_pct"),
    ("INDIAINFO", "momentum", "momentum_primary_train/trades.csv",
     "single", "entry_date", "exit_date", "net_pnl_pct"),
    ("JINDALSTEL", "momentum", "momentum_primary_train/trades.csv",
     "single", "entry_date", "exit_date", "net_pnl_pct"),
    ("JINDALSTEL", "same_sector_pairing", "same_sector_pairing/random_primary_train.csv",
     "pairs", "entry_fill_date", "exit_fill_date", "long_net_pct"),
]

REPEAT_SYMBOLS = ["RUSHIL", "INDIAINFO", "IBREALEST", "COREPROTEC", "JINDALSTEL"]
...
def main() -> int:
    ...
    rows = []
    for symbol, strategy, rel_path, kind, date_col, exit_col, ret_col in CELLS:
        df = pd.read_csv(RUNS / rel_path, parse_dates=[date_col, exit_col])
        if kind == "single":
            sub = df[(df["symbol"] == symbol) & df[ret_col].notna()]
        else:
            sub = df[(df["long_symbol"] == symbol) & df[ret_col].notna()]

        for _, r in sub.iterrows():
            entry, exit_ = r[date_col], r[exit_col]
            own_ret = float(r[ret_col])
            nifty_window_ret = (
            ...
            rows.append({
                "symbol": symbol, "strategy": strategy, "entry_date": entry, "exit_date": exit_,
    ...
    out_dir = RUNS / "top_performer_diagnosis"
    out_dir.mkdir(parents=True, exist_ok=True)
    out.to_csv(out_dir / "trade_level_regime.csv", index=False)

    print("\n=== PER-TRADE DETAIL ===")
    print(out[["symbol", "strategy", "entry_date", "own_return_pct", "excess_vs_nifty_pct",
               "excess_vs_sector_pct", "nifty_trailing_63d_return_pct",
               "stress_composite_at_entry"]].to_string(index=False))

    print("\n\n=== PER-SYMBOL SUMMARY (mean across all its winning trades) ===")
    summary = out.groupby("symbol").agg(
        ...
        pct_beat_sector=("excess_vs_sector_pct", lambda s: 100 * (s > 0).mean()),
        pct_beat_nifty=("excess_vs_nifty_pct", lambda s: 100 * (s > 0).mean()),
    ).reset_index()
    print(summary.to_string(index=False))
    summary.to_csv(out_dir / "per_symbol_summary.csv", index=False)

    # population baseline: same stats, but pooled over ALL trades in each of
    # the source strategies (not just these 5 stocks), to know whether the
    # per-symbol numbers above are unusual or typical
    print("\n\n=== FULL-HISTORY BUY-AND-HOLD CHECK (own stock vs NIFTY50, "
          "over the full span its winning trades occurred in) ===")
    bh_rows = []
    for symbol in REPEAT_SYMBOLS:
        sym_trades = out[out["symbol"] == symbol]
        if sym_trades.empty or symbol not in stock_close.columns:
            continue
        span_start = sym_trades["entry_date"].min()
        span_end = sym_trades["exit_date"].max()
