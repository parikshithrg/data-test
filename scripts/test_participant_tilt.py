"""Phase 3, signal 3: participant-flow regime tilt, tested under this
project's rules.

    python scripts/test_participant_tilt.py --split delivery --window train

Same harness as the other Phase 3 scripts, with one structural difference:
`dtest.data.participant_flow` returns a single market-wide daily Series
(FII net index-futures OI has no per-symbol breakdown), reindexed onto the
price panel's calendar only - not per-symbol like `delivery_breakout`/
`oi_momentum`. `participant_tilt_signal` itself computes `mean_reversion_
signal` internally and gates it, so this script does not call
`mean_reversion_signal` directly.

Runs on `delivery` - FII participant data begins 2018-01-01, inside that
split's own train_start (2019-06-27), so no window needs to be invented.
`primary`/train (2004-2016) would produce zero signals by construction and
...
STORY = (
    ...
    "into. The identical dip bought while FII positioning sits below its "
    "own recent trend is disproportionately the early stage of a real "
    "breakdown - buying it fights the flow that actually moves the "
    "market. Since NSE's participant-OI report has no per-stock "
    "breakdown, this can only gate WHETHER any dip is bought on a given "
    "day, market-wide, uniformly across every symbol - it cannot select "
    "WHICH stock, that is still mean_reversion_signal's job."
)
...
def main() -> int:
    ...
    print(f"=== participant_tilt ({args.split}/{args.window}) ===")
    ...
    print("loading FII net index-futures flow from fno.db ...")
    fii_net = load_fii_net_index_flow(cfg.paths.fno_db)
    print(f"FII flow series: {len(fii_net)} sessions "
         f"({fii_net.index[0].date()} .. {fii_net.index[-1].date()})")
    ...
    fii_aligned = fii_net.reindex(stock_close.index)
    coverage = fii_aligned.notna().mean()
    print(f"  FII flow coverage after reindex: {coverage:.1%} of sessions "
         f"(zero by construction before 2018-01-01)")
    ...
    out_dir = Path(cfg.paths.runs) / f"participant_tilt_{args.split}_{args.window}"
    ...
    entry = HypothesisEntry(
        title="participant_tilt (honest execution)", story=STORY,
