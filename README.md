...
| Macro cross-asset stress | `scripts/fetch_macro_stress_series.py` (raw CSVs, no loader module) | varies by series, all through 2026-08-25/26 | Original 4 (US VIX, USD/INR, DXY, gold) built for the now-closed/rejected cross-asset stress-gate test (`dtest/features/stress.py`, commit `71222274`). Extended 2026-08-26 (dataset priority queue Tier 6 item 16) with 8 more, purely as raw material for the next analysis phase, not wired into any signal yet: WTI crude, an EM-FX basket proxy (CEW), a US 10Y yield proxy (`^TNX`, substituting for FRED — every FRED endpoint refused connections all session during the item-13 macro work) and 4 global equity reads (S&P 500, Euro Stoxx 50, Nikkei, Hang Seng) plus broad EM equity (EEM). All 8 live-probed via `yf.download` before being added — none were dead ends. Crude's one non-positive close (2020-04-20, -$37.63) is the real WTI negative-price event, not a data bug |
...
| AMC monthly portfolio disclosures (SBI + Axis) | `dtest/data/amc_portfolios.py` | SBI: 2013-11 to 2026-07, 68 workbooks, 109MB. Axis: 2021-09 to 2026-07, 3,672 workbooks, 296MB | SBI's `GetSchemePortfolioSheets` and Axis's `cms/get-scheme-documents` (behind a STATIC, non-session public bearer token from `cms/token`) both return everything via plain `requests` — no browser/session needed for either, contradicting the 2026-08-24 "needs browser automation" verdict for Axis specifically (that finding was about the page UI, not the underlying data API). ICICI Prudential (needs real click-driven scraping, each URL only exists in a React `onClick` handler) and HDFC (data is trivial static links, but the site sits behind a real Akamai bot wall) were also scoped live and genuinely DO need real browser automation — the per-AMC cost is real, just not universal. Raw workbooks only; a holdings-table parser (SBI: ~120 scheme sheets/workbook, merged headers; Axis: one scheme per workbook, simpler) is real, unbuilt follow-on work. SBI has a genuine unexplained gap 2016-05 to 2023-01; Axis's own pre-2021 filings use a completely different, non-standardized naming era, correctly excluded rather than mis-parsed |
| SBI + Axis equity holdings (per-stock) | `dtest/data/amc_holdings.py` | Axis: 2021-09 to 2026-07 (full). SBI: 2023-01 to 2026-07 only (see caveat). 243,224 rows, 199 schemes, 52 months, 34MB | Real per-stock buy/sell/hold data — the actual point of this item, not just a filing manifest. ONE shared parser for both AMCs (same SEBI-mandated section vocabulary and row shape confirmed independently on two completely different templates), scoped to EQUITY holdings only (debt/money-market/derivatives sections have a different column shape, deliberately out of scope). `pct_to_nav` normalized to a fraction for both AMCs — Axis stores it as one already, SBI stores it as a percentage (confirmed live, not assumed). Two real file formats both named `.xls` on both AMCs (legacy BIFF needs `xlrd`, modern XLSX-zip needs `openpyxl` via an in-memory buffer — `openpyxl` rejects real ZIP content outright if the path ends in `.xls`, a real trap). A genuine double-counting bug (13 Axis manifest rows sharing a case-insensitive-identical filename on Windows) was caught by auditing summed `pct_to_nav` per scheme-month — several came out near 2.0 before the fix, none after (max ~1.01-1.08). **Real, stated gap**: SBI's 2013-2016 files use a different, narrower 6-column holdings table this parser doesn't yet handle — combined with the already-known 2016-2023 filing gap, SBI's real coverage here is 2023-2026 only, not its full 2013-2026 file range |
...
4. AMC monthly portfolio disclosures — **partially done, 2026-08-26**: 4
   AMCs scoped live, 2 fully fetched. SBI (largest by AUM) and Axis MF
   (smaller) both need NO browser automation — SBI's file list loads via
   one plain endpoint found by reading the site's own JS source; Axis's
   loads via a JSON API behind a STATIC public bearer token (found via a
   hooked `fetch`, confirmed non-session by its own 9-month-old
   `updatedAt`). 68 SBI + 3,672 Axis real workbooks fetched (109MB +
   296MB), raw files only — the holdings-table parser is real, unbuilt
   follow-on work. ICICI Prudential DOES need real click-driven
   interaction (each file URL only exists in a React click handler); HDFC
   has trivially simple data but sits behind a real Akamai bot wall that
   blocks all non-browser traffic. AUM rank predicts nothing about
   scraping cost — the biggest and a mid-sized AMC were cheap, the 2nd and
   3rd largest were each expensive for different reasons
...
13. ~~Macro series (RBI repo/M3/forex, MOSPI IIP/CPI/GDP, GST
    collections)~~ — **done, with one real scope drop**: MOSPI's IIP/CPI/
    WPI/GDP/PLFS-unemployment and RBI forex reserves via `api.mospi.gov.in`;
    GST monthly collections from GSTN's own "9 Years of GST" PDF report;
    a repo-rate proxy (call-money rate) and bank credit growth from FRED
    (see the Data sources table above). **M3 dropped from scope** — no
    current source found anywhere (every FRED variant dead since
    2018-2023; RBI's own Handbook is bot-walled and DBIE is an
    unreachable JS-only SPA)
14. ~~Global cross-asset (FRED rates, crude, EM-FX, global equity indices)~~ —
    done, 2026-08-26. 8 series added to `scripts/fetch_macro_stress_series.py`:
    WTI crude, an EM-FX basket proxy (CEW), a US 10Y yield proxy (`^TNX`,
    substituting for FRED entirely), and 5 global equity reads (S&P 500,
    Euro Stoxx 50, Nikkei, Hang Seng, broad EM). Raw data only, not yet fed
    into any signal — this closes the dataset priority queue's Tier 1-6
    collection phase in full (only item 4, AMC portfolios, remains
