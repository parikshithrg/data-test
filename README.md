...
| Axis Mutual Fund equity holdings (per-stock) | `dtest/data/amc_holdings.py` | 2021-09 to 2026-07, 116,284 rows, 101 schemes, 52 months, 17MB | Real per-stock buy/sell/hold data — the actual point of this item, not just a filing manifest. Scoped to EQUITY holdings only (the debt/money-market/derivatives sections have a different column shape, deliberately out of scope). Two real file formats both named `.xls` (legacy BIFF needs `xlrd`, modern XLSX-zip needs `openpyxl` via an in-memory buffer — `openpyxl` rejects real ZIP content outright if the path ends in `.xls`, a real trap). A genuine double-counting bug (13 manifest rows sharing a case-insensitive-identical filename on Windows) was caught by auditing summed `pct_to_nav` per scheme-month — several came out near 2.0 before the fix, none after (max 1.01) |
